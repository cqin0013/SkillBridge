...
const STATE_COLUMN_MAP = {
  ...
  ACT: 'Fieldaustralian_capital_territory_shortage_rating',
};
...
export default function initAnzscoDemandRoutes(pool) {
  ...
  // Unify the return body of "all states"
  ...
  */
  router.get('/:code/demand', async (req, res) => {
    ...
    try {
      ...
      // Without state: return all states
      if (!state) {
        return res.json(shapeAll(meta, row));
      }

      // With state: only return the state
